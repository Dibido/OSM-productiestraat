...
struct Message
{
	...
	struct MessageHeader
	{
		...
		std::string toString() const
		{
			...
			os << magicNumber1 << magicNumber2 << magicNumber3 << magicNumber4
					<< majorVersion << minorVersion << messageType
					<< std::setw(
							10 /* unsigned long : 4,294,967,295 ergo 10 numbers */)
					<< messageLength;
			return os.str();
		}
		/**
		...
		void fromString(const std::string& aString)
		{
			...
			char major;
			char minor;
			is >> magic[0] >> magic[1] >> magic[2] >> magic[3] >> major >> minor
					>> messageType
					>> std::setw(
							10 /* unsigned long : 4,294,967,295 ergo 10 numbers */)
					>> messageLength;
	...
	Message(char aMessageType, const std::string& aMessage) :
			messageType(aMessageType), message(aMessage)
	{
	}
	/**
	 *
	...
	Message(const Message& aMessage) :
			messageType(aMessage.messageType), message(aMessage.message)
	{
	}
	/**
	 *
